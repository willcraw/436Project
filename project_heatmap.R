...
make_plot = function(df, given_type){
  df %>% drop_na() %>% filter(year %in% c(2019, 2020, 2021, 2022), type == given_type, Country %in% c("India", "China", "United States", "Indonesia", "Pakistan", "Nigeria", "Brazil", "Bangladesh", "Russian Federation", "Ethiopia", "Mexico", "Japan", "Egypt, Arab Rep.", "Philippines", "Congo, Dem. Rep.", "Vietnam", "Iran, Islamic Rep.", "Turkey", "Germany", "Thailand")) %>%  ggplot() + geom_tile(aes(x=year, y=Country, fill=rate)) + scale_fill_gradient(low="white", high = "red", limits = c(-10, 100)) + labs(fill=given_type)
}
